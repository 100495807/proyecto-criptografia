...
class UserManager:
    def __init__(self):
        self.certificates_folder = "certificados"
        self.security_manager = SecurityManager()

    def register_user(self, username, email, hashed_password, salt, phone, gender, address, user_type):
        ...
        # Generar claves RSA
        private_key, public_key = self.security_manager.generate_rsa_key_pair()
        encrypted_private_key = self.security_manager.encrypt_private_key(private_key, hashed_password, nonce)
    ...
    def delete_songs_by_user_id(self, user_id):
        conn = self.security_manager.create_connection()
        cursor = conn.cursor()
    ...
    def verify_email_recovery(self, email):
        conn = self.security_manager.create_connection()
        cursor = conn.cursor()
    ...
    def update_password(self, user_id, hashed_password, salt):
        ...
        conn = self.security_manager.create_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET hashed_password = ?, salt = ? WHERE id = ?", (hashed_password, salt, user_id))
        conn.commit()
        conn.close()
    ...
    def get_user_certificate(self, user_id):
        ...
        cert_path = os.path.join(self.certificates_folder, f"{username}_cert.pem")
