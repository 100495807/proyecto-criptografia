...
from security import create_connection, sign_comment, verify_comment_signature, decrypt_aes_gcm, derive_key
...
def add_comment(user_id, song_name, author_name, comment, private_key_pem, password, salt):
    ...
    cursor = conn.cursor()
    cursor.execute('INSERT INTO comments (user_id, song_name, author_name, comment, signature) VALUES (?, ?, ?, ?, ?)',
...
def get_comments(song_id=None):
    ...
    cursor = conn.cursor()
    if song_id:
        cursor.execute('SELECT user_id, song_name, author_name, comment, signature FROM comments WHERE song_id = ?', (song_id,))
    else:
